...
VMenu.f:Boolean("AQ", "Auto Q farm, hold 'V'", true)
...
function Combo(unit)
	if IOW:Mode() == "Combo" then
		...
		castE(unit)
			...
			if VMenu.c.W:Value() and CanUseSpell(myHero,_W) == READY and ValidTarget(unit, 725) then
			...
			if VMenu.c.Q:Value() and Ready(0) and ValidTarget(unit, 725) then
...
function FarmQ()
	if VMenu.f.AQ:Value() and Ready(_Q) and IOW:Mode() == "LaneClear" then
